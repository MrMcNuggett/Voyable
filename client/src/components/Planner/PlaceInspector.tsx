...
export function ActionButton({ onClick, variant, icon, label }: ActionButtonProps) {
  const base = {
    ...
    danger: { background: 'rgba(239,68,68,0.08)', color: 'var(--danger)', border: 'none', hoverBg: 'rgba(239,68,68,0.16)' },
  }
        ...
        {activeMembers.map(member => {
          ...
          return (
            <div key={member.id}
              ...
              className={isHovered && canRemove ? 'bg-[rgba(239,68,68,0.06)] text-[var(--danger)]' : 'bg-surface-hover text-content'}
              style={{
