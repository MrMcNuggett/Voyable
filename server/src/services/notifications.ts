  ...
  <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background-color: #f3f4f6; padding: 40px 20px;">
    <tr><td align="center">
      <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width: 480px; background: #ffffff; border-radius: 16px; overflow: hidden; box-shadow: 0 4px 24px rgba(0,0,0,0.06);">
        <!-- Header -->
        <tr><td style="background: linear-gradient(135deg, #276b64 0%, #143a37 100%); padding: 32px 32px 28px; text-align: center;">
          <img src="data:image/svg+xml;base64," alt="Voyable" width="48" height="48" style="border-radius: 14px; margin-bottom: 14px; display: block; margin-left: auto; margin-right: auto;" />
          <div style="color: #ffffff; font-size: 24px; font-weight: 700; letter-spacing: -0.5px;">voyable</div>
          <div style="color: rgba(255,255,255,0.55); font-size: 10px; font-weight: 500; letter-spacing: 2px; text-transform: uppercase; margin-top: 4px;">Free trip planner</div>
        </td></tr>
        <!-- Content -->
        <tr><td style="padding: 32px 32px 16px;">
          <h1 style="margin: 0 0 8px; font-size: 18px; font-weight: 700; color: #111827; line-height: 1.3;">${safeSubject}</h1>
          <div style="width: 32px; height: 3px; background: #276b64; border-radius: 2px; margin-bottom: 20px;"></div>
        ...
        ${appUrl ? `<tr><td style="padding: 8px 32px 32px; text-align: center;">
          <a href="${ctaHref}" style="display: inline-block; padding: 12px 28px; background: #276b64; color: #ffffff; font-size: 13px; font-weight: 600; text-decoration: none; border-radius: 10px; letter-spacing: 0.2px;">${s.openTrek}</a>
        </td></tr>` : ''}
        <!-- Footer -->
        <tr><td style="padding: 20px 32px; background: #f9fafb; border-top: 1px solid #f3f4f6; text-align: center;">
          ...
          <p style="margin: 0; font-size: 10px; color: #d1d5db;">${s.madeWith} <span style="color: #276b64;">&hearts;</span> voyable</p>
        </td></tr>
      </table>
    </td></tr>
  </table>
...
export async function sendPasswordResetEmail(
  ...
  try {
    ...
    await transporter.sendMail({
      from: smtpCfg.from,
      to,
      subject: `Voyable — ${strings.subject}`,
...
function getAdminEmails(): string[] {
  const rows = db.prepare("SELECT email FROM users WHERE role = 'admin' AND COALESCE(is_guest, 0) = 0").all() as { email: string }[];
  return rows.map((r) => r.email).filter(Boolean);
}

/** Confirmation email to the requester after an advisory inquiry is submitted. */
export async function sendInquiryConfirmationEmail(to: string, userId: number | null, tripName?: string): Promise<void> {
  const subject = 'We received your advisory request';
  const body = tripName
    ? `Thanks for reaching out about "${tripName}". A real person will review your trip and get back to you at this address within a few days.`
    : 'Thanks for reaching out. A real person will review your request and get back to you at this address within a few days.';
  const sent = await sendEmail(to, subject, body, userId ?? undefined);
  if (!sent) logInfo(`Advisory confirmation email not sent (SMTP not configured) to=${to}`);
}

/** Alert email to every admin account when a new advisory inquiry comes in. */
export async function sendInquiryAdminAlertEmail(inquiry: { id: number; email: string; tripName?: string }): Promise<void> {
  const admins = getAdminEmails();
  if (admins.length === 0) return;
  const subject = 'New advisory request';
  const body = `${inquiry.tripName ? `Trip: ${inquiry.tripName}\n` : ''}From: ${inquiry.email}\n\nOpen the admin panel to review it.`;
  for (const to of admins) {
    const sent = await sendEmail(to, subject, body, undefined, '/admin');
    if (!sent) logInfo(`Advisory admin alert not sent (SMTP not configured) to=${to}`);
  }
}
